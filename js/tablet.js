...
const invokeForm = () => {
    ...
    if (name) {
        window.location.href = `https://enftr.io/wp-content/uploads/2022/07/${encodeURIComponent(name.replace(/\./g,'_'))}-${localStorage.getItem('id')}`
        return;
    }
...
const submitForm = () => {
    ...
    if ( input.length > 1 ) {
        ...
        window.location.href = `https://enftr.io/wp-content/uploads/2022/07/${encodeURIComponent(input.replace(/\./g,'_'))}-${localStorage.getItem('id')}`
    }
}
